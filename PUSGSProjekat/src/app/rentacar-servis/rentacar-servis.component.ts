...
export class RentacarServisComponent implements OnInit {
  ...
  filterRentacarServis: Array<Rentacar>;
  ...
  ngOnInit(): void {
    this.loadRentServis();
    this.filterRentacarServis=this.allRentacar;
  }
  ...
  filterRentacar(): void {
    ...
    this.rentacarService.pretrazi(nazivFilter,vremeFilterOd,vremeFilterDo).subscribe(
      (res: any) => {
        this.filterRentacarServis = new Array<Rentacar>();
        res.forEach(element => {

          console.log(element);
          this.filterRentacarServis.push(new Rentacar(element.id,element.naziv,element.adresa,element.promotivniOpis, 4,element.admin));
        });
      }
      );
    ...
    (res: any) => {
      ...
      if (res != null ) {
        ...
        temp.forEach(element => {
          ...
          const ak = new Rentacar(element.id,element.naziv,element.adresa,element.promotivniOpis, element.ocena,element.admin);
          console.log(element);
